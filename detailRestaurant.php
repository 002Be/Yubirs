	...
	<body data-spy="scroll" data-bs-target="#secondary_nav" data-offset="75">
		<?php
			if(isset($_GET["sepeteeklendi"])){
				if(isset($_SESSION["sepet"])==null){
					...
					$_SESSION["sepetPartner"] = $_GET["sepeteeklen"];
				}else{
					if(!strstr($_SESSION["sepet"], $_GET["sepeteeklendi"])){
						$_SESSION["sepet"] = $_SESSION["sepet"]."|".$_GET["sepeteeklendi"];
					}
					if(!strstr($_SESSION["sepetPartner"], $_GET["sepeteeklen"])){
						$_SESSION["sepetPartner"] = $_SESSION["sepetPartner"]."|".$_GET["sepeteeklen"];
					}
				}
			}
			// echo $_SESSION["sepetPartner"]."<br>";
			// echo $_SESSION["sepet"];
		?>
		<!-- Header -->
		<?php headerC(); ?>
		...
		<main>
			...
			if($restoranBilgileri["imageId"]!="Yok"){ ?>
				<div class="hero_in detail_page background-image" data-background="url(assets/partnerBusinessImg/<?php echo $restoranBilgileri["imageId"]; ?>)"><?php
			}else{ ?>
				<div class="hero_in detail_page background-image" data-background="url(assets/img/hero_general.jpg)"> <?php
			} ?>
								...
								<div class="col-xl-8 col-lg-7 col-md-6 position-relative">
									<div class="buttons clearfix">
										<span class="magnific-gallery">
											<?php
											if($restoranBilgileri["imageId"]!="Yok"){ ?>
												<a href="assets/partnerBusinessImg/<?php echo $restoranBilgileri["imageId"]; ?>" class="btn_hero" title="<?php echo $restoranBilgileri["businessName"]; ?>" data-effect="mfp-zoom-in"><i class="icon_image"></i>Resmi gör</a> <?php
											}else{ ?>
												<a href="assets/img/detail_1.jpg" class="btn_hero" title="<?php echo $restoranBilgileri["businessName"]; ?>" data-effect="mfp-zoom-in"><i class="icon_image"></i>Resmi gör</a> <?php
											} ?>
										</span>
										<?php
										if(isset($_SESSION["Customer"])){ ?>
										<a href="#0" class="btn_hero wishlist"><i class="icon_heart"></i>Favoriler</a> <?php
										} ?>
									</div>
								</div>
							</div>
							...
							<section id="section-1">
								...
								<div class="row">
									...
									<div class="col-md-6" id="<?php echo $menuler["id"]; ?>">
										<a class="menu_item" href="detailRestaurant.php?id=<?php echo $_GET["id"]; ?>&sepeteeklendi=<?php echo $menuler["id"]; ?>&sepeteeklen=<?php echo $menuler["partnerId"]; ?>#<?php echo $menuler["id"]; ?>" style="text-decoration: none;">
										<!-- <a class="menu_item modal_dialog" href="#modal-dialog" title="<?php echo $menuler["description"]; ?>"> -->
											<figure>
												<?php
												if($menuler["imageId"]!="Yok"){ ?>
													<img src="assets/partnerMealImg/<?php echo $menuler["imageId"]; ?>" alt="Yok"> <?php
												}else{ ?>
													<img src="assets/img/menu-thumb-placeholder.jpg" data-src="assets/img/menu-thumb-1.jpg" alt="thumb" class="lazy"> <?php
												} ?>
											</figure>
											<!-- < ?php if($menuler["discount"]!=0){ ?> <span class="off" style="position:absolute; bottom:0; right:0; ">%< ?php echo $menuler["discount"]; ?></span> < ?php } ?> -->
